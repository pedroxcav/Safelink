...
class RelatoServiceTest {
    ...
    @Test
    @DisplayName("Criar relato com dados válidos")
    void createRelato_ok() {
        NewRelatoDTO dto = new NewRelatoDTO(
                TipoGolpe.GOLPE_DO_PRESENTE,
                TipoCanal.WHATSAPP,
                "Descrição",
                LocalDate.now(),
                TipoDado.SITE,
        ...
        service.createRelato(dto, token);

        verify(relatoRepository).save(any(Relato.class));
    }
